...
import { Send, Loader2, Mic, StopCircle, AlertCircle, Volume2, VolumeX } from 'lucide-react';
...
const OllamaChat: React.FC<OllamaChatProps> = ({ chartData, demoMode = false }) => {
  ...
  const { isListening, startListening, stopListening, listenForResult, speak, stopSpeaking, isSpeaking } = useSpeech();
          ...
          >
            ...
            >
              ...
              {message.role === 'assistant' && message.id && (
                <button
                  className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-full hover:bg-background/80"
